...
class Model(object):
    def __init__(self, config ,graph):
        with graph.as_default():
            ...
            self.decode_method = config["decode_method"]# crf解码
            self.num_tags = config["num_tags"]#17
            self.num_chars = config["num_chars"] #1284
            self.num_segs = 4 #
            self.bias = config['bias'] # 10
            ...
            self.dropout = tf.placeholder(dtype=tf.float32,
                                          name="Dropout")
            # tf.sign return -1 if x<0 0 x==0 1x>0做mask
            used = tf.sign(tf.abs(self.char_inputs))
            # 在第二维上求和，得到输入的最长length
            ...
            if self.model_type == 'bilstm':
                ...
                self.model_outputs = self.biLSTM_layer(model_inputs, self.lstm_dim, self.lengths)
                if self.decode_method=='crf':
                    # logits for tags
                    self.logits = self.project_layer_bilstm(self.model_outputs)
                elif self.decode_method=='lstm':
                    self.logits = self.lstm_decode_layer(self.model_outputs)
    ...
    def embedding_layer(self, char_inputs, seg_inputs, config, name=None): # 返回字符和seg的拼接embedding
        """
    ...
    def loss_layer(self, project_logits, lengths, name=None):
        ...
        with tf.variable_scope("crf_loss"  if not name else name):
            ...
            return tf.reduce_mean(-log_likelihood)+lossL2
    ...
    def run_step(self, sess, type, batch):
        ...
        if type=='train':
            ...
            lengths, logits, lstm_outputs = sess.run([self.lengths, self.logits,self.model_outputs], feed_dict)
            return lengths, logits, lstm_outputs
    ...
    def evaluate(self, sess, data_manager, id_to_tag, type):
        ...
        for batch in data_manager.iter_batch():
            ...
            tags = batch[-1]
            if type=='test':
                lengths, logits, _ = self.run_step(sess, type, batch)
            else:
    ...
    def evaluate_line(self, sess, inputs, id_to_tag):
        """
        输出命名实体识别模型输出的tag解码标签，字符embedding，和双向lstm的输出
        :param sess:
        :param inputs:
        :param id_to_tag:
        :return:
        """
        lengths, logits, lstm_outputs = self.run_step(sess, 'test', inputs)
        if self.decode_method == 'crf':
        ...
        return tags, self.char_lookup.eval(session=sess), lstm_outputs
